...
export default function CursosAnunciosScreen() {
    ...
    const fetchAnuncios = async () => {
        try {
            ...
            const anunciosGrupo = await obtenerAnunciosByGrupo(grupo);


            // Ordenar por fecha de publicación (más recientes primero)
            anunciosGrupo.sort((a, b) => new Date(b.fechaPublicacion) - new Date(a.fechaPublicacion));
            setAnuncios(anunciosGrupo);
        } catch (error) {
    ...
    if(anuncios.length === 0) {
        return <Text style={styles.noAnuncios} >No se encontraron anuncios.</Text>;
    }
