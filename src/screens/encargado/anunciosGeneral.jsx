...
export function AnunciosScreen() {
    ...
    const fetchAnuncios = async () => {
        try {
            ...
            // 📌 Obtener los estudiantes a cargo
            const estudiantes = await getEstudiantesByEncargado(userId);
            
            let anunciosTemp = [];
            let processedGroups = new Set(); // Para evitar duplicados

            for (const estudiante of estudiantes) {
                const grupos = await getGruposByEstudiante(estudiante.id);

                for (const grupo of grupos) {
                    if (!processedGroups.has(grupo.idGrupoCurso)) {
                        processedGroups.add(grupo.idGrupoCurso);
